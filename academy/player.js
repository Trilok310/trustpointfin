/**
 * TrustPoint Finance Academy — Interactive Course Player (V3.0 Full-Stack Auth)
 * Candidate OTP Authentication & Server-Authoritative Progression Engine
 * Client is NEVER trusted for authorization. All unlocking & grading is server-verified.
 */
...
class AcademyPlayer {
  constructor() {
    ...
    // Candidate Identity (Session-bound)
    this.candidate = null;
    ...
    // Auth DOM elements
    this.authModal = document.getElementById('auth-modal-backdrop');
    this.authStepMobile = document.getElementById('auth-step-mobile');
    this.authStepOtp = document.getElementById('auth-step-otp');
    this.inputMobile = document.getElementById('input-mobile');
    this.inputOtp = document.getElementById('input-otp');
    this.authErrorBox = document.getElementById('auth-error-box');
    this.candidateStatusEl = document.getElementById('candidate-auth-status');
    this.candidateMobileEl = document.getElementById('candidate-mobile-display');
  ...
  async checkAuth() {
    try {
      ...
      console.warn('[ACADEMY AUTH] Could not check session:', e);
    }
  ...
  showAuthModal() {
    if (this.authModal) {
      ...
      this.switchAuthStep('mobile');
      if (this.inputMobile) this.inputMobile.focus();
    }
  }
  ...
  switchAuthStep(step) {
    this.clearAuthError();
    if (step === 'mobile') {
      if (this.authStepMobile) this.authStepMobile.style.display = 'flex';
      if (this.authStepOtp) this.authStepOtp.style.display = 'none';
      if (this.inputMobile) this.inputMobile.focus();
    } else {
      if (this.authStepMobile) this.authStepMobile.style.display = 'none';
      if (this.authStepOtp) this.authStepOtp.style.display = 'flex';
      const displaySent = document.getElementById('display-sent-mobile');
      if (displaySent && this.inputMobile) displaySent.innerText = '+91 ' + this.inputMobile.value.trim();
      if (this.inputOtp) {
        this.inputOtp.value = '';
        this.inputOtp.focus();
      }
  ...
  startResendCountdown(seconds) {
    ...
    if (secEl) secEl.innerText = remaining + 's';
  ...
  async requestOtp() {
    this.clearAuthError();
    const rawMobile = this.inputMobile ? this.inputMobile.value.trim() : '';
    const cleanMobile = rawMobile.replace(/[^0-9]/g, '');
    if (!cleanMobile || cleanMobile.length !== 10 || !/^[6-9]/.test(cleanMobile)) {
      this.setAuthError('कृपया एक वैध 10-अंकीय भारतीय मोबाइल नंबर दर्ज करें (शुरुआत 6, 7, 8 या 9 से)।');
      return;
    }
    ...
    if (btnReq) {
      ...
      btnReq.innerHTML = '<span>ओटीपी भेजा जा रहा है...</span> <i class="fa-solid fa-circle-notch fa-spin"></i>';
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.stagingOtp && this.inputOtp) {
          console.log('[STAGING OTP]', data.stagingOtp);
          this.inputOtp.value = data.stagingOtp;
        }
      } else {
        this.setAuthError(data.error || 'ओटीपी भेजने में असमर्थ। कृपया पुनः प्रयास करें।');
      }
    } catch (e) {
      this.setAuthError('सर्वर से कनेक्शन विफल रहा। कृपया नेटवर्क जांचें।');
    } finally {
      if (btnReq) {
        btnReq.disabled = false;
        btnReq.innerHTML = '<span>ओटीपी प्राप्त करें (Get OTP)</span> <i class="fa-solid fa-arrow-right"></i>';
      }
    }
  }
  ...
  async verifyOtp() {
    this.clearAuthError();
    const cleanMobile = this.inputMobile ? this.inputMobile.value.trim().replace(/[^0-9]/g, '') : '';
    const otp = this.inputOtp ? this.inputOtp.value.trim() : '';

    if (!otp || otp.length !== 6 || !/^\d{6}$/.test(otp)) {
      this.setAuthError('कृपया 6-अंकीय संख्यात्मक ओटीपी दर्ज करें।');
      return;
    }
    ...
    if (btnVerify) {
      ...
      btnVerify.innerHTML = '<span>सत्यापित हो रहा है...</span> <i class="fa-solid fa-circle-notch fa-spin"></i>';
    }
    ...
    try {
      ...
      if (data.success && data.candidate) {
        ...
        this.setAuthError(data.error || 'अमान्य ओटीपी। कृपया पुनः प्रयास करें।');
      }
    } catch (e) {
      this.setAuthError('सर्वर से कनेक्शन त्रुटि।');
    } finally {
      if (btnVerify) {
        btnVerify.disabled = false;
        btnVerify.innerHTML = '<span>सत्यापित करें और शुरू करें (Verify &amp; Start)</span> <i class="fa-solid fa-shield-halved"></i>';
      }
    }
  }
  ...
  updateCandidateUI() {
    if (this.candidate && this.candidateStatusEl) {
      this.candidateStatusEl.style.display = 'flex';
      if (this.candidateMobileEl) {
        const m = this.candidate.mobile;
        this.candidateMobileEl.innerText = m.length >= 10 ? `+91 ${m.slice(0,2)}***${m.slice(-4)}` : m;
      }
    } else if (this.candidateStatusEl) {
      this.candidateStatusEl.style.display = 'none';
    }
  }
  ...
  async init() {
    ...
    if (auth.authenticated && auth.candidate) {
      this.candidate = auth.candidate;
      this.updateCandidateUI();
    } else {
      this.showAuthModal();
    ...
    // 2. Fetch authoritative state from the server
    await this.fetchAuthoritativeProgress();

    // 3. Fetch lesson educational data
    await this.fetchLessonData();

    // 4. Bind UI events & Render
    this.bindEvents();
    this.renderCurrentScreen();
    this.updateCurriculumDrawer();
  }
  ...
  async fetchAuthoritativeProgress() {
    try {
      ...
      console.warn('[ACADEMY SECURITY] Server unreachable. Enforcing strict default lock:', err);
    }
    this.unlockedLessons = ['M01_L01'];
    this.completedLessons = [];
  }
  ...
  async fetchLessonData(targetLessonId = null) {
    ...
    try {
      ...
      const apiRes = await fetch(`/api/academy/lessons/${lessonKey}`, { credentials: 'include' });
      ...
      const response = await fetch(`data/${fileName}`, { credentials: 'include' });
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        if (submitBtn) {
          ...
          submitBtn.innerText = 'सबमिट करें ✓';
        }
        return;
      }
      ...
      alert('सर्वर से संपर्क नहीं हो सका। कृपया सुनिश्चित करें कि आप लॉग इन हैं और बैकएंड सक्रिय है।');
      if (submitBtn) {
        submitBtn.disabled = false;
