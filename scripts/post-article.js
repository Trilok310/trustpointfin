...
const INSIGHTS_PATH = path.join(PUBLIC_DIR, "insights.html");
const TEMPLATE_PATH = path.join(PUBLIC_DIR, "sample-insight.html");
...
async function main() {
  ...
  const finalHtmlPath = path.join(PUBLIC_DIR, `${expectedSlug}.html`);
  ...
  if (fs.existsSync(stagedHtmlPath) && fs.existsSync(stagedPendingPath)) {
      ...
      if (stagedPending.topic !== currentTopic || stagedPending.filename !== (expectedSlug + ".html")) {
          ...
          if (fs.existsSync(path.join(STAGING_DIR, "sitemap.tmp.xml"))) fs.renameSync(path.join(STAGING_DIR, "sitemap.tmp.xml"), SITEMAP_PATH);
...
---END---`;
  ...
  fs.renameSync(tmpSitemap, SITEMAP_PATH);
  fs.renameSync(stagedPendingPath, PENDING_PATH);
